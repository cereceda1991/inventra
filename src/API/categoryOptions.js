export const categoryOptions = [
  { value: 'limpieza', description: 'Limpieza' },
  { value: 'cuidado_personal', description: 'Cuidado Personal' },
  { value: 'galletas_y_golosinas', description: 'Galletas y golosinas' },
  { value: 'bebidas', description: 'Bebidas' },
  { value: 'abarrotes', description: 'Abarrotes' },
  { value: 'cereales', description: 'cereales' },
  { value: 'snacks', description: 'Snacks' },
  { value: 'frutas_verduras', description: 'Frutas y Verduras' },
  { value: 'carnes', description: 'Carnes y Pescados' },
  { value: 'panaderia', description: 'Panadería' },
  { value: 'productos_limpieza', description: 'Productos de Limpieza' },
  { value: 'maquillaje', description: 'Maquillaje' },
  { value: 'perfumes', description: 'Perfumes' },
  { value: 'shampoo_acondicionador', description: 'Shampoo y Acondicionador' },
  { value: 'jabones', description: 'Jabones' },
  { value: 'detergentes', description: 'Detergentes' },
  { value: 'cepillos_peines', description: 'Cepillos y Peines' },
  { value: 'cuidado_piel', description: 'Cuidado de la Piel' },
  { value: 'cuidado_cabello', description: 'Cuidado del Cabello' },
  { value: 'cuidado_bucal', description: 'Cuidado Bucal' },

  // Agrega más opciones según tus necesidades
];
